...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    ###################### Good practice to use tf_getVariable for weight sharing between GPUs !!!!!
    
    
    W = tf.Variable(tf.zeros([Config.n_features,Config.n_classes],dtype=tf.float32))
    b = tf.Variable(tf.zeros([1,Config.n_classes],dtype=tf.float32))
    
    y_hat = softmax(tf.matmul(input_data,W) + b)
    #raise NotImplementedError
    ### END YOUR CODE
    out = y_hat
