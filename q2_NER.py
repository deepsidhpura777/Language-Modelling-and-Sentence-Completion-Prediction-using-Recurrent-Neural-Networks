...
class Config(object):
  ...
  label_size = 5
  hidden_size = 100
  max_epochs = 24 ########### Changed from 24 to 1
  early_stopping = 2
  dropout = 0.7
  lr = 0.001
  l2 = 0.001
...
class NERModel(LanguageModel):
  ...
  def load_data(self, debug=False):
    ...
    print ("Word Vector lenght",len(self.wv))
    print ("Shape",self.X_train.shape)
    print ("Row",self.X_train[1,:])
  ...
  def add_placeholders(self):
    ...
    self.dropout_placeholder = tf.placeholder(tf.float32,shape = (),name="dropouts")
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope('Embeddings'):
          embeddings = tf.get_variable('embedding',shape=[len(self.wv),self.config.embed_size],initializer=xavier_weight_init())          
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('Layer'):
        W = tf.get_variable("W_Layer",shape=[self.config.window_size * self.config.embed_size,self.config.hidden_size],initializer=xavier_weight_init())
        b1 = tf.get_variable("b_Layer",shape=[self.config.hidden_size],initializer=tf.constant_initializer(0.0))
        
    with tf.variable_scope('Softmax'):
        U = tf.get_variable("U_Softmax",shape=[self.config.hidden_size,self.config.label_size],initializer=xavier_weight_init())
        b2 = tf.get_variable("b_Softmax",shape=[self.config.label_size],initializer=xavier_weight_init())
    
    
    
    h = tf.nn.tanh(tf.matmul(window,W) + b1)
    h = tf.nn.dropout(h,self.dropout_placeholder)
    y_hat = tf.matmul(h,U) + b2 ##### Dont calculate softmax right now because the loss funciton will apply softmax !
    y_hat = tf.nn.dropout(y_hat,self.dropout_placeholder)
    
    l2_loss = tf.nn.l2_loss(W) + tf.nn.l2_loss(b1) + tf.nn.l2_loss(U) + tf.nn.l2_loss(b2)    #### Adding losses of all the layers !!!!
  ...
  def add_loss_op(self, y):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred,labels=target)) + self.config.l2 * tf.get_collection("l2_loss")[0] #### 0 index important as tf.get_collection returns  vector    
    
    ########### You can either take the sum or the average loss !!!! Here the average loss is taken !!!!!!!!
