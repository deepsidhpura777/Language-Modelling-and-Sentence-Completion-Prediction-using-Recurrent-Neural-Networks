...
def softmax(x):
  ...
  ### YOUR CODE HERE
  max_x = tf.reduce_max(x,reduction_indices=[1])
  max_shape = tf.shape(max_x)
  max_x = tf.reshape(max_x,[max_shape[0],1])
  x = x - max_x
  exp_x = tf.exp(x)
  denom = tf.reduce_sum(exp_x,reduction_indices=[1])
  denom_shape = tf.shape(denom)
  denom = tf.reshape(denom,[denom_shape[0],1])
  answer = exp_x / denom
...
#          print "Denom_shape",sess.run(tf.shape(denom))
...
def cross_entropy_loss(y, yhat):
  ...
  ce = tf.reduce_sum(ce,reduction_indices =[0])
#==============================================================================
#   with tf.Session() as sess:
#       print (sess.run(ce))
#   
