...
class Config(object):
  ...
  hidden_size = 100
  num_steps = 10
  max_epochs = 100  ####### changes from 16
  early_stopping = 2
  dropout = 0.9
  lr = 0.001
...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'): 
        with tf.variable_scope('Embedding_Layer') as scope:
            embeddings = tf.get_variable('embedding',shape=[len(self.vocab),self.config.embed_size],initializer=xavier_weight_init(),trainable=True)          
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('projection'):
        ...
        b_2 = tf.get_variable("b_2",shape=[len(self.vocab)],initializer=tf.constant_initializer(0.0))
  ...
  def add_loss_op(self, output):
    ...
    ######### Need to reshape the labels placeholder !!!!!
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('dropout'):
        ...
        for i in inputs:
            temp = tf.nn.dropout(i,self.dropout_placeholder)
    ...
    with tf.variable_scope('initial'):
        ...
        H = tf.get_variable("H_Layer",shape=[self.config.hidden_size,self.config.hidden_size],initializer=xavier_weight_init())
        I = tf.get_variable("I_Layer",shape=[self.config.embed_size,self.config.hidden_size],initializer=xavier_weight_init())
        b_1 = tf.get_variable("b_2_Layer",shape=[self.config.hidden_size],initializer=tf.constant_initializer(0.0))
        ...
        for i in range(len(new_inputs)):
            ...
            h = tf.sigmoid(tf.matmul(self.initial_state,H) + tf.matmul(temp,I) + b_1)
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ### YOUR CODE HERE
    feed = {model.input_placeholder: [tokens[-1:]],   ##### need to go in the reverse order !!!
  ...
  with tf.Session() as session:
    ...
    for epoch in xrange(config.max_epochs):
      ...
      if valid_pp < best_val_pp:
        ...
        saver.save(session, '/Users/student/Downloads/Assignment 2 RNN/rnn_weights')
      if epoch - best_val_epoch > config.early_stopping:
        break
      print 'Total time: {}'.format(time.time() - start)
      
    saver.restore(session, '/Users/student/Downloads/Assignment 2 RNN/rnn_weights')
